...
// Должен вызываться из всех точек входа
//
// Параметры:
//	Параметр1 	- Тип1 - 
//
...
&НаСервере
Процедура Мод_ЗагрузитьЗаказы(ИспДатыАктуальности = Неопределено, ДатаИзменений = Неопределено, Знач ИДИзменений = Неопределено) Экспорт
...
Процедура Мод_СозданиеЗаказов(МасивЗаказов, ТЗброни = Неопределено) Экспорт
...
Процедура РегистрацияСкладскойСистемыНаСервере(СкладскаяСистемаАктивна = Ложь) Экспорт
	...
	Если СкладскаяСистемаАктивна = Истина Тогда 
		//ничего не делаем
	Иначе 		
		Если ЗначениеЗаполнено(ИмяСервера) Тогда     //ЕНС. добавил проверку на заполненность
...
Процедура УТ11_СоздатьЗаказ(ДанныеЗаказа, стр, искДок, ТЗброни = Неопределено) Экспорт
	...
	//ЕНС. Заказ будет использоваться как счет всегда
	...
	//КонецЕсли;
	Для каждого строк Из Док.Товары Цикл
		строк.ВариантОбеспечения = Перечисления.ВариантыОбеспечения.НеТребуется;
	КонецЦикла;
	...
	// ЕНС. Установка свойств заказа в "Дополнительные сведения"
	
	СвойствоЗаказов = crm_RetailCRMОбработка.УТ11_ПолучитьСвойство("ИД",                    // это ключ для поиска заказа в 1С
		Справочники.НаборыДополнительныхРеквизитовИСведений.УдалитьДокумент_ЗаказКлиента);
	СвойствоЗаказовМагазин = crm_RetailCRMОбработка.УТ11_ПолучитьСвойство("site", 
		Справочники.НаборыДополнительныхРеквизитовИСведений.УдалитьДокумент_ЗаказКлиента);
