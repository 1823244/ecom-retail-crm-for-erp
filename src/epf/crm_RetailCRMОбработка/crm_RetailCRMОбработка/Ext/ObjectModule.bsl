...
Процедура Мод_ЗагрузитьЗаказы(ИспДатыАктуальности = Неопределено, ДатаИзменений = Неопределено, ИДИзменений = Неопределено) Экспорт
	...
	Если crm_RetailCRMПовтИсп.ПолучитьЗначениеКонстанты("ЗагружатьЗаказы") = Ложь Тогда Возврат КонецЕсли;
...
Процедура Мод_СозданиеЗаказов(МасивЗаказов, ТЗброни = Неопределено)
	...
	Для каждого стр из МасивЗаказов Цикл
		...
		СоздаватьВозвраты = crm_RetailCRMПовтИсп.ПолучитьЗначениеКонстанты("СозданиеВозвратов");
...
Процедура УТ11_СоздатьСтрокуДокумента(док,стр,ОтменаЗаказа = Ложь, Выше11_5_7_390, Эталонная, ТекСтр, СоответствиеСтатусов)
	...
	//
	Если crm_RetailCRMПовтИсп.ПолучитьЗначениеКонстанты("ИспользованиеПЛ") Тогда 
...
Процедура ПолучитьТоварыИзRetailCRM () Экспорт
	...
	Для каждого Товар Из МассивТоваров Цикл
		...
		СоздатьОбновитьНоменклатуру(Товар);
...
//
// Параметры:
//	Товар 	- Соответствие - получаем из json
//
Процедура СоздатьОбновитьНоменклатуру(Товар)
	...
	Если _м.Количество() > 0 Тогда
		...
		Если НЕ ЗначениеЗаполнено(СтруктураВозврата.искНоменклатура) 
			...
			СоздатьНоменклатуру(Товар);
			
		Иначе 
			ОбновитьНоменклатуру(СтруктураВозврата, Товар);
		КонецЕсли;
...
//
// Параметры:
//	СтруктураОбъекта	- Соответствие - из json RetailCRM
//
// Возвращаемое значение:
//	Тип: Тип_значения
//
Функция СоздатьНоменклатуру(СтруктураОбъекта, jsonText = "") Экспорт
	
	xmlIdТовара = Неопределено;
	_м = СтруктураОбъекта["offers"];
	Если _м.Количество() > 0 Тогда
		xmlIdТовара = _м[0]["xmlId"];
	КонецЕсли;	
	Если xmlIdТовара = Неопределено Тогда
		сообщить("Не удалось получить xmlIdТовара! Товар не создан! name = "+строка(СтруктураОбъекта["name"]));
		Возврат Ложь;
	КонецЕсли;
	...
	ЗаполнитьРеквизитыЭлемента(ОбъектДанных, СтруктураОбъекта);
...
//	Параметр1 	- Тип1 - 
//
Процедура ЗаполнитьРеквизитыЭлемента(ОбъектДанных, СтруктураОбъекта) Экспорт
...
//	Параметр1 	- Тип1 - 
//
Процедура ОбновитьНоменклатуру(СтруктураВозврата, СтруктураОбъекта)
