...
Процедура Мод_ЗагрузитьЗаказы(ИспДатыАктуальности = Неопределено, ДатаИзменений = Неопределено, ИДИзменений = Неопределено) Экспорт
	...
	Если crm_RetailCRMОбщий.ПолучитьЗначениеКонстанты("ЗагружатьЗаказы") = Ложь Тогда Возврат КонецЕсли;
	
	ИДИзменений = crm_RetailCRMОбщий.ПолучитьЗначениеКонстанты("since_Id");
	
	ДатаЗагрузки = ТекущаяДата();
	
	Отказ = Ложь;
	Если СокрЛП(crm_RetailCRMОбщий.ПолучитьЗначениеКонстанты("ИмяСервера")) = "" Тогда
		Сообщить("Не указано имя сервера в настройках.");
		ЗаписьЖурналаРегистрации("RetailCRM",,,,"Не указано имя сервера." + ДатаЗагрузки);
		Отказ = Истина;
	КонецЕсли;
	
	Если СокрЛП(crm_RetailCRMОбщий.ПолучитьЗначениеКонстанты("КлючCRM")) = "" Тогда
		Сообщить("Не указан ключ api в настройках.");
	...
	Если ИспДатыАктуальности = Истина Тогда //и ДатаИзменений <> Дата(1,1,1) Тогда          //если стоит флажок загрузки по дате
		Если ДатаИзменений = Неопределено или ДатаИзменений = "" или ДатаИзменений = Дата(1,1,1) Тогда 
			Сообщить("Не указана дата актуальности!");
			Возврат;
		Иначе
	...
	Иначе                                           //загрузка по sinceId
		ЗаписьЖурналаРегистрации("RetailCRM",,,,"Загрузка заказов по дате актуальности: " + ИДИзменений);
		Если ИДИзменений = 0 Тогда 
			Сообщить("Не указан id последних изменений. Укажите id или загрузите по дате актуальности, id заполнится автоматически");
			Возврат;
		КонецЕсли;
		фильтрИд = "";
	...
	Если crm_RetailCRMОбщий.ПолучитьЗначениеКонстанты("ЗагрузкаБроней") = Истина Тогда
		packs_sinceId = crm_RetailCRMОбщий.ПолучитьЗначениеКонстанты("packs_sinceId");
	...
	Иначе 
		Сообщить("За данный период изменения в RetailCRM отсутствовали!");
		Возврат
	КонецЕсли;
	
	Сообщить(" - Запрос к retailCRM завершён - ");
...
Процедура Мод_ВыполнитьОбмен() Экспорт
	...
	ОбменОстатками 		= crm_RetailCRMОбщий.ПолучитьЗначениеКонстанты("ВыгружатьОстатки");
	ЗагружатьЗаказы 	= crm_RetailCRMОбщий.ПолучитьЗначениеКонстанты("ЗагружатьЗаказы");
	ВыгружатьТипыЦен 	= crm_RetailCRMОбщий.ПолучитьЗначениеКонстанты("ВыгружатьТипыЦен");
...
Функция ФормированиеСтруктурыЗаказа(стр)
	...
	Если стр["delivery"]["time"]["from"] = Неопределено
		ИЛИ стр["delivery"]["time"]["to"] = Неопределено Тогда
	...
	Если crm_RetailCRMОбщий.ПолучитьЗначениеКонстанты("КорпоративныеКлиенты") = Истина Тогда 
...
Процедура Мод_СозданиеЗаказов(МасивЗаказов, ТЗброни = Неопределено)
	...
	НаборСтатусов 	= crm_RetailCRMОбщий.ПолучитьЗначениеКонстанты("ТабСтатусов");
	...
	Для каждого стр из МасивЗаказов Цикл
		...
		искДок = УТ11_ВернутьДокументЗаказ (Строка(стрЗаказа["id"]));
		
		СоздаватьВозвраты = crm_RetailCRMОбщий.ПолучитьЗначениеКонстанты("СозданиеВозвратов");
		...
		Если ЗначениеЗаполнено(искДок) Тогда  
			...
			Если стр["status"] = "partially-returned" или стр["status"] = "return" или СоздВозврат = Истина Тогда
				...
				ДокРеализация = УТ11_ВернутьРеализацию(искДок.Ссылка);
				Если ЗначениеЗаполнено(ДокРеализация) и ДокРеализация.Проведен Тогда 
					...
					//если нашли возврат, отменяем и перезаполняем
					Если ЗначениеЗаполнено(ДокВозврат) и докВозврат.Проведен Тогда
						ДокВозвратОб = ДокВозврат.ПолучитьОбъект();
						ДокВозвратОб.Записать(РежимЗаписиДокумента.ОтменаПроведения);
					КонецЕсли;
					
					УТ11_СоздатьВозврат(ДокВозврат, ДокРеализация, искДок, стрЗаказа.Товары);
...
&НаСервере
Функция УТ11_ВернутьДокументЗаказ (ИД)
	...
	СвойствоЗаказов = Обработки.crm_RetailCRMОбработка.УТ11_ПолучитьСвойство("ИД", 
...
Функция УТ11_ВернутьДокументОплаты (ИД)
	...
	СвойствоОплат.Добавить(Обработки.crm_RetailCRMОбработка.УТ11_ПолучитьСвойство("ИДоплаты", Справочники.НаборыДополнительныхРеквизитовИСведений.УдалитьДокумент_ПоступлениеБезналичныхДенежныхСредств));
	СвойствоОплат.Добавить(Обработки.crm_RetailCRMОбработка.УТ11_ПолучитьСвойство("ИДоплаты", Справочники.НаборыДополнительныхРеквизитовИСведений.УдалитьДокумент_ПриходныйКассовыйОрдер));
	СвойствоОплат.Добавить(Обработки.crm_RetailCRMОбработка.УТ11_ПолучитьСвойство("ИДоплаты", Справочники.НаборыДополнительныхРеквизитовИСведений.УдалитьДокумент_ОперацияПоПлатежнойКарте));
...
Функция УТ11_ОпределениеКонтрагента(стр,организация)
	...
	КорпоративныеКлиенты =  crm_RetailCRMОбщий.ПолучитьЗначениеКонстанты("КорпоративныеКлиенты");
	...
	КонецЕсли;
	искПарт.ГруппаДоступа = crm_RetailCRMОбщий.ПолучитьЗначениеКонстанты("ГруппаДоступаПартнеров");
	искПарт.Комментарий = "#";
	...
	Обработки.crm_RetailCRMОбработка.УТ11_УстановитьСвойствоУОбъекта(искКонт.Ссылка, СвойствоКонтрагентов, idКонтрагента);  
	//
	...
	Обработки.crm_RetailCRMОбработка.УТ11_УстановитьСвойствоУОбъекта(искПарт.Ссылка, СвойствоПартнеров, idПартнера);  
	//
	...
	Если КорпоративныеКлиенты и типКлиента = "customer_corporate" Тогда 
		...
		СвойствоКонтЛица = Обработки.crm_RetailCRMОбработка.УТ11_ПолучитьСвойство("ИД", Справочники.НаборыДополнительныхРеквизитовИСведений.УдалитьСправочник_КонтактныеЛицаПартнеров);
		Обработки.crm_RetailCRMОбработка.УТ11_УстановитьСвойствоУОбъекта(искКонтЛицо.Ссылка, СвойствоКонтЛица, ДанныеКонтактногоЛица["ID"]);  
		//
	КонецЕсли;
...
Функция УТ11_ВернутьКонтактноеЛицоПоID (ИД)
	...
	Запрос.УстановитьПараметр("Значение", ИД);
	СвойствоКонтЛиц = Обработки.crm_RetailCRMОбработка.УТ11_ПолучитьСвойство("ИД", Справочники.НаборыДополнительныхРеквизитовИСведений.УдалитьСправочник_КонтактныеЛицаПартнеров);
...
Процедура ЗаписатьДокумент(Док)
	...
	Попытка
		...
		Сообщить("Документ проведен: " + Док , СтатусСообщения.Информация);
	Исключение
		Сообщить("Не удалось провести документ: " + Док + " " + ОписаниеОшибки(), СтатусСообщения.Важное);
		
		Попытка
			Док.Записать(РежимЗаписиДокумента.Запись);
			Сообщить("Документ записан: " + Док , СтатусСообщения.Информация);
		Исключение
			Сообщить("Не удалось записать документ: " + Док + " " + ОписаниеОшибки(), СтатусСообщения.Важное);
		КонецПопытки;
	КонецПопытки;
...
&НаСервере
Процедура УТ11_СоздатьЗаказ(ДанныеЗаказа, стр, искДок, ТЗброни = Неопределено)
	
	Эталонная = crm_RetailCRMОбщий.ПолучитьЗначениеКонстанты("Эталонная");
	
	НесколькоОрганизаций = Ложь;
	
	Если ЗначениеЗаполнено(ДанныеЗаказа.КодМагазина)Тогда 
		Если crm_RetailCRMОбщий.ПолучитьЗначениеКонстанты("НесколькоОрганизаций") = Истина Тогда 
	...
	Если Организация = Неопределено Тогда 
		Организация = crm_RetailCRMОбщий.ПолучитьЗначениеКонстанты("Организация");
	КонецЕсли;
	...
	Если не ЗначениеЗаполнено(искДок) Тогда
		...
		док.Приоритет			=  ?(НесколькоОрганизаций = Истина, crmОрганизация.Приоритет, crm_RetailCRMОбщий.ПолучитьЗначениеКонстанты("Приоритет"));
		док.Организация 		= Организация;
		док.НалогообложениеНДС 	=  ?(НесколькоОрганизаций = Истина, crmОрганизация.Налогообложение, crm_RetailCRMОбщий.ПолучитьЗначениеКонстанты("Налогообложение"));
		док.ЦенаВключаетНДС    = ?(НесколькоОрганизаций = Истина, crmОрганизация.ЦенаВключаетНДС, crm_RetailCRMОбщий.ПолучитьЗначениеКонстанты("ЦенаВключаетНДС"));
		Если НесколькоОрганизаций = Истина Тогда 
			Валюта			= crmОрганизация.Валюта;
		Иначе 
			Валюта			= crm_RetailCRMОбщий.ПолучитьЗначениеКонстанты("Валюта");
		КонецЕсли;
		док.Валюта 				= Валюта;
	...
	Иначе
		...
		Если СостояниеЗаказа = Перечисления.СостоянияЗаказовКлиентов.Закрыт Тогда 
			Сообщить("Заказ №" + искДок.Номер + " закрыт!!!");
			Возврат
		КонецЕсли;
	КонецЕсли;
	...
	ИспользованиеЗаказов = crm_RetailCRMОбщий.ПолучитьЗначениеКонстанты("ИспользованиеЗаказов");
	...
	Если Эталонная Тогда
		...
		ТаблицаСтатусов = crm_RetailCRMОбщий.ПолучитьЗначениеКонстанты("ТабСтатусов");
	...
	Если ЗначениеЗаполнено(СкладОтг) Тогда 
		СписокСкладов = crm_RetailCRMОбщий.ПолучитьЗначениеКонстанты("Склады");
	...
	Если Не ЗначениеЗаполнено(Склад) Тогда 
		...
		Иначе 
			Склад = crm_RetailCRMОбщий.ПолучитьЗначениеКонстанты("Склад");
		КонецЕсли;
	КонецЕсли;
	
	КлиентПартнер 	= УТ11_ОпределениеКонтрагента(ДанныеЗаказа,организация);
	
	док.Партнер 			= КлиентПартнер.Получить("Партнер");
	док.Контрагент 			= КлиентПартнер.Получить("Клиент");
	док.Склад 				= Склад;
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьСоглашенияСКлиентами") Тогда
		...
		Иначе 
			док.Соглашение = crm_RetailCRMОбщий.ПолучитьЗначениеКонстанты("ТиповоеСоглашение");
		КонецЕсли;
	...
	Если Не ЗначениеЗаполнено(БанковскийСчет) Тогда 
		...
		Иначе 
			БанковскийСчет = crm_RetailCRMОбщий.ПолучитьЗначениеКонстанты("БанковскийСчет");
		КонецЕсли;
	КонецЕсли;
	...
	Если не ЗначениеЗаполнено(Касса) Тогда 
		...
		Иначе 
			Касса = crm_RetailCRMОбщий.ПолучитьЗначениеКонстанты("ОсновнаяКасса");
		КонецЕсли;
	КонецЕсли;	
	...
	Если ДанныеЗаказа.СпособДоставки <> Неопределено Тогда 
		СписокДоставок = crm_RetailCRMОбщий.ПолучитьЗначениеКонстанты("СоответствиеДоставок");
	...
	Если ДанныеЗаказа["КодМенеджера"]<> Неопределено Тогда  
		СписокМенеджеров = crm_RetailCRMОбщий.ПолучитьЗначениеКонстанты("СоответствиеМенеджеров");
	...
	Если ОтменаЗаказа = Ложь Тогда  
		...
		//	Иначе 
		//		СтрокаОплата.Сдвиг =  Формат(crm_RetailCRMОбщий.ПолучитьЗначениеКонстанты("ОтсрочкаДней"),"ЧЦ=10; ЧДЦ=0");
		//	КонецЕсли;
		//Иначе 
	...
	Если НЕ ИспользованиеЗаказов = "ЗаказКакСчет" Тогда
		Если Не Эталонная Тогда 
			Если СтатусЗаказа = "for-shipment" или СтатусЗаказа = "otgruzhen" или СтатусЗаказа = "complete"  Тогда 
				Для каждого строк Из Док.Товары Цикл
					строк.ВариантОбеспечения = Перечисления.ВариантыОбеспечения.Отгрузить;		
				КонецЦикла;
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
	...
	Исключение
		...
		Исключение
			Сообщить("Не удалось записать: " + Док + " " + ОписаниеОшибки(), СтатусСообщения.Важное);
		КонецПопытки;
	КонецПопытки;
		
	СвойствоЗаказов = Обработки.crm_RetailCRMОбработка.УТ11_ПолучитьСвойство("ИД", Справочники.НаборыДополнительныхРеквизитовИСведений.УдалитьДокумент_ЗаказКлиента);
	СвойствоЗаказовМагазин = Обработки.crm_RetailCRMОбработка.УТ11_ПолучитьСвойство("site", Справочники.НаборыДополнительныхРеквизитовИСведений.УдалитьДокумент_ЗаказКлиента);
	Обработки.crm_RetailCRMОбработка.УТ11_УстановитьСвойствоУОбъекта(Док.Ссылка, СвойствоЗаказов, ДанныеЗаказа["id"]);  
	Обработки.crm_RetailCRMОбработка.УТ11_УстановитьСвойствоУОбъекта(Док.Ссылка, СвойствоЗаказовМагазин, ДанныеЗаказа["site"]);
	
	//Если заказ оплачен, то на каждую оплату создается документ оплаты 
	//тип документы зависит от типа оплаты
	
	Если crm_RetailCRMОбщий.ПолучитьЗначениеКонстанты("СозданиеОплат") = Истина Тогда 
		...
		Для Каждого Оплата из ДанныеЗаказа["Оплаты"] Цикл  
			...
			ТипДокументаОплаты = "";
			Терминал = "";
			СписокОплат = crm_RetailCRMОбщий.ПолучитьЗначениеКонстанты("СоответствиеТиповОплат");
			Если СписокОплат <> Неопределено Тогда 
				...
				Если Оплата["СтатусОплаты"] = "paid" Тогда  			
					УТ11_СоздатьДокументОплаты(док, Оплата, ТипДокументаОплаты, ДокументОплаты, Касса, Терминал); 
	...
	Иначе 
		Если СтатусЗаказа = "for-shipment" или СтатусЗаказа = "otgruzhen" или СтатусЗаказа = "complete" Тогда  
			Если НесколькоОрганизаций = Истина Тогда
				...
				КонецЕсли;
			Иначе
				Если crm_RetailCRMОбщий.ПолучитьЗначениеКонстанты("СозданиеРеализаций") = Истина Тогда 
					СоздаватьРеализацию = Истина;
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
	...
	Если СоздаватьРеализацию  Тогда 	
		...
		Запрос.УстановитьПараметр("ЗаказКлиента", док.ссылка);
		...
		//если есть реализация то сначала делаем отмену проведения
		Если ЗначениеЗаполнено(Реализация) Тогда          //если есть реализация или уже стоит пометка удаление то идем дальше
			Если Реализация.Проведен Тогда
				РеализацияОб = Реализация.ПолучитьОбъект();
				РеализацияОб.Записать(РежимЗаписиДокумента.ОтменаПроведения);
			КонецЕсли;
		...
		УТ11_СоздатьРеализацию(док, Реализация, СтатусЗаказа);
		
	КонецЕсли;	
	
	//если все позиции номенклатуры заполнены, выгружаем id товаров
...
Процедура ВыгрузитьЗаказВCRM(док, НомерР)     
	...
	переменные.Вставить("apiKey",crm_RetailCRMОбщий.ПолучитьЗначениеКонстанты("КлючCRM"));
	...
	ответ = crm_RetailCRMОбщий.HTTPзапросPOST(crm_RetailCRMОбщий.ПолучитьЗначениеКонстанты("ИмяСервера")
...
// Процедура - Создать реализацию
// В данной версии шаблонного решения процедура не используется
// Параметры:
//  док	 - 	Заказ клиента - 
//
Процедура УТ11_СоздатьРеализацию(док, Реализация, СтатусЗаказа)
	...
	СвойствоРеализаций = Обработки.crm_RetailCRMОбработка.УТ11_ПолучитьСвойство("crm_СтатусЗаказа", Справочники.НаборыДополнительныхРеквизитовИСведений.УдалитьДокумент_РеализацияТоваровУслуг, "Сведение");
	Обработки.crm_RetailCRMОбработка.УТ11_УстановитьСвойствоУОбъекта(докР.Ссылка, СвойствоРеализаций, СтатусЗаказа);  
	
	Если (СтатусЗаказа = "complete" или СтатусЗаказа = "partially-completed") и докР.Проведен тогда
		
		СостояниеЗаказа = УТ11_ПолучитьСостояниеЗаказа(док);
		
		Если СостояниеЗаказа = Перечисления.СостоянияЗаказовКлиентов.ГотовКЗакрытию Тогда 
			//закрываем заказ
			докОб = док.ПолучитьОбъект();
			докОб.Статус = Перечисления.СтатусыЗаказовКлиентов.Закрыт;
			Попытка
				докОб.Записать(РежимЗаписиДокумента.Проведение);
				Сообщить("Заказ закрыт: " + докОб , СтатусСообщения.Информация);
			Исключение
				Сообщить("Не удалось закрыть заказа: " + докОб + " " + ОписаниеОшибки(), СтатусСообщения.Важное);
			КонецПопытки;
		КонецЕсли;
	КонецЕсли;
...
Процедура УТ11_СоздатьДокументОплаты(док, Оплата, ТипДокументаОплаты, ДокументОплаты, Касса, Терминал)
	...
	Если ТипДокументаОплаты = "Поступление безналичных ДС" Тогда 
		...
		СвойствоОплаты = Обработки.crm_RetailCRMОбработка.УТ11_ПолучитьСвойство("ИДоплаты", Справочники.НаборыДополнительныхРеквизитовИСведений.УдалитьДокумент_ПоступлениеБезналичныхДенежныхСредств);
	...
	ИначеЕсли ТипДокументаОплаты = "Приходный кассовый ордер" Тогда
		...
		СвойствоОплаты = Обработки.crm_RetailCRMОбработка.УТ11_ПолучитьСвойство("ИДоплаты", Справочники.НаборыДополнительныхРеквизитовИСведений.УдалитьДокумент_ПриходныйКассовыйОрдер);
		СвойствоТип = Обработки.crm_RetailCRMОбработка.УТ11_ПолучитьСвойство("ТипОплаты", Справочники.НаборыДополнительныхРеквизитовИСведений.УдалитьДокумент_ПоступлениеБезналичныхДенежныхСредств);
	...
	ИначеЕсли ТипДокументаОплаты = "Эквайринговая операция" и ПолучитьФункциональнуюОпцию("ИспользоватьОплатуПлатежнымиКартами")= Истина Тогда
		...
		СвойствоОплаты = Обработки.crm_RetailCRMОбработка.УТ11_ПолучитьСвойство("ИДоплаты", Справочники.НаборыДополнительныхРеквизитовИСведений.УдалитьДокумент_ОперацияПоПлатежнойКарте);
	...
	Если докОБ <> Неопределено Тогда
		...
		Обработки.crm_RetailCRMОбработка.УТ11_УстановитьСвойствоУОбъекта(докОБ.Ссылка, СвойствоОплаты, Формат(Оплата["ИДОплаты"], "ЧГ=0"));
		
		Обработки.crm_RetailCRMОбработка.УТ11_УстановитьСвойствоУОбъекта(докОБ.Ссылка, СвойствоТип, Оплата["ТипОплаты"]);
...
&НаСервере
Процедура УТ11_СоздатьСтрокуДокумента(док,стр,ОтменаЗаказа = Ложь, Выше11_5_7_390, Эталонная, ТекСтр, СоответствиеСтатусов)
	
	//ищем строку по тч товары по идентификатору строки, если найдем, то редактируем ее
	offers_id = crm_RetailCRMОбщий.ПолучитьЗначениеКонстанты("offers_id");
	...
	Иначе 
		...
		//ИдентификаторТовара
		ИдентифТовара = crm_RetailCRMОбщий.ПолучитьЗначениеКонстанты("ИдентификаторТовара");
	...
	Если стр["ТипЦены"] <> "" Тогда 
		Если crm_RetailCRMОбщий.ПолучитьЗначениеКонстанты("ИспользованиеТиповЦен") = Истина Тогда
	...
	//
	Если crm_RetailCRMОбщий.ПолучитьЗначениеКонстанты("ИспользованиеПЛ") Тогда 
		Для Каждого Скидка из стр["Скидки"] Цикл 
			Если Скидка["type"] = "bonus_charge" Тогда 
				НовСтр.crm_СкидкаПЛ = Скидка["amount"]; 
			КонецЕсли;
		КонецЦикла;
		Если стр["НачисленоБонусов"] <> Неопределено Тогда 
			НовСтр.crm_НачисленоПЛ = стр["НачисленоБонусов"];
		КонецЕсли;
	КонецЕсли;
	//
		...
		Если Выше11_5_7_390 Тогда
			...
			Если Эталонная Тогда
				Если ИспользованиеЗаказов = "ЗаказТолькоСоСклада" Тогда 
					...
					ИначеЕсли 
						док.Статус = Перечисления.СтатусыЗаказовКлиентов.КОтгрузке Тогда 
						новСтр.ВариантОбеспечения 	= Перечисления.ВариантыОбеспечения.Отгрузить;
					КонецЕсли;
				Иначе 	
					Если стр["СтатусТовара"] = "completed" или  стр["СтатусТовара"] = "ready-for-assembly" или
						стр["СтатусТовара"] = "sold" Тогда
						новСтр.ВариантОбеспечения 	= Перечисления.ВариантыОбеспечения.Отгрузить;
					ИначеЕсли 
						стр["СтатусТовара"] = "in-reserve" Тогда
						новСтр.ВариантОбеспечения 	= Перечисления.ВариантыОбеспечения.СоСклада;
					ИначеЕсли 
						стр["СтатусТовара"] = "confirming" или стр["СтатусТовара"] = "new" Тогда
						новСтр.ВариантОбеспечения 	= Перечисления.ВариантыОбеспечения.кОбеспечению;
					КонецЕсли;
				КонецЕсли;	
			КонецЕсли; 
...
//			новСтр.ВариантОбеспечения 	= Перечисления.ВариантыОбеспечения.Требуется;
	...
	НовСтр.crm_idПозиции = стр["ИДСтроки"];
...
Процедура УТ11_СравнитьСоставЗаказа(док, ТоварыCRM, ОтменаЗаказа, Выше11_5_7_390, Эталонная, ТЗброни, СоответствиеСтатусов) 
	...
	ИспользованиеХарактеристик = crm_RetailCRMОбщий.ПолучитьЗначениеКонстанты("ИспользованиеХарактеристик");
	GUIDОрганизации = crm_RetailCRMОбщий.ПолучитьЗначениеКонстанты("GUIDОрганизации");
	offers_id = crm_RetailCRMОбщий.ПолучитьЗначениеКонстанты("offers_id");
	...
	Для Счетчик = 0 по Колво Цикл
		стр1С = док.Товары.Получить(ИндексСтр); 
		Если ИспользованиеХарактеристик = Истина Тогда
			xmlIdТовара = СокрЛП(стр1С.Номенклатура.УникальныйИдентификатор());
			xmlIdТовара = ?(ЗначениеЗаполнено(стр1С.Характеристика), 
				xmlIdТовара + "#" + СокрЛП(стр1С.Характеристика.УникальныйИдентификатор()), xmlIdТовара);
		...
		Иначе 
			//Идентификатор товара
			Если ИдентифТовара = "Код" Тогда
				xmlIdТовара = СокрЛП(стр1С.Номенклатура.Код);
			ИначеЕсли ИдентифТовара = "Артикул" Тогда
				xmlIdТовара	= СокрЛП(стр1С.Номенклатура.Артикул);
			ИначеЕсли ИдентифТовара = "GUID" Тогда
				xmlIdТовара	= СокрЛП(стр1С.Номенклатура.УникальныйИдентификатор());	
			КонецЕсли;
		КонецЕсли;
		...
		НайденСтр = ТЗcrm.НайтиСтроки(Новый Структура("xmlId", xmlIdТовара));
		Если НайденСтр <> Неопределено и НайденСтр.Количество()<>0 Тогда
			Если НайденСтр.Количество() > 0 Тогда 
				//ищем по идентификатору
				НайденСтрока = ТЗcrm.Найти(стр1С.ИдентификаторСтроки, "ИдентификаторСтроки1С");
				Если НайденСтрока <> Неопределено Тогда
					УТ11_СоздатьСтрокуДокумента(док,НайденСтрока, ОтменаЗаказа, Выше11_5_7_390, Эталонная, стр1С, СоответствиеСтатусов);
					ТЗcrm.Удалить(ТЗcrm.Индекс(НайденСтрока));	
				Иначе 
					док.Товары.Удалить(стр1С);
				КонецЕсли;
			КонецЕсли;
		Иначе 
			док.Товары.Удалить(стр1С);
	...
	//теперь идем по тз crm
	
	Индекс = ТЗcrm.Количество() - 1;
	Пока Индекс >= 0 Цикл
		Если ЗначениеЗаполнено(ТЗcrm[Индекс].ИдентификаторСтроки1С) Тогда 
			НайденСтрока1С = док.Товары.Найти(ТЗcrm[Индекс].ИдентификаторСтроки1С, "ИдентификаторСтроки");
			Если НайденСтрока1С <> Неопределено Тогда
				УТ11_СоздатьСтрокуДокумента(док,ТЗcrm[Индекс], ОтменаЗаказа, Выше11_5_7_390, Эталонная, НайденСтрока1С, СоответствиеСтатусов);
				ТЗcrm.Удалить(ТЗcrm[Индекс]); 
				Индекс = Индекс - 1;
			Иначе 
				//добавляем строку в док.Товары
				УТ11_СоздатьСтрокуДокумента(док,ТЗcrm[Индекс], ОтменаЗаказа, Выше11_5_7_390, Эталонная, Неопределено, СоответствиеСтатусов);
		...
		Иначе
			...
			НайденСтрока1С = док.Товары.Найти(ТЗcrm[Индекс].ИДСтроки, "crm_idПозиции");
			Если НайденСтрока1С <> Неопределено Тогда
				УТ11_СоздатьСтрокуДокумента(док,ТЗcrm[Индекс], ОтменаЗаказа, Выше11_5_7_390, Эталонная, НайденСтрока1С, СоответствиеСтатусов);
				ТЗcrm.Удалить(ТЗcrm[Индекс]); 
				Индекс = Индекс - 1; 
...
Функция УТ11_ВернутьНоменклатуру(xmlIdТовара) Экспорт 
	...
	Если crm_RetailCRMОбщий.ПолучитьЗначениеКонстанты("GUIDОрганизации") = Истина Тогда 
...
Процедура УТ11_Услуги(док,стр)
	...
	Если стр["СтоимостьДоставки"]<>Неопределено Тогда
		Если ЗначениеЗаполнено(стр["СтоимостьДоставки"]) и стр["СтоимостьДоставки"]<>"0" Тогда  
			...
			стрУслуги.Номенклатура			= crm_RetailCRMОбщий.ПолучитьЗначениеКонстанты("Доставка");
			...
			стрУслуги.ВариантОбеспечения 	= Перечисления.ВариантыОбеспечения.НеТребуется;
...
Процедура Мод_ОстаткиИЦеныВCRM() Экспорт 
	...
	Иначе
		...
		НовСтр = СписокМагазинов.Добавить();
		НовСтр.КодМагазина		= crm_RetailCRMОбщий.ПолучитьЗначениеКонстанты("КодМагазина");
		НовСтр.Организация		= crm_RetailCRMОбщий.ПолучитьЗначениеКонстанты("Организация");
	...
	Иерархия = crm_RetailCRMОбщий.ПолучитьЗначениеКонстанты("ИерархияНоменклатуры");
	...
	Для Каждого Магазин Из СписокМагазинов Цикл 
		...
		Иначе
			Если Иерархия = "Виды номенклатуры" Тогда
				Родитель = crm_RetailCRMОбщий.ПолучитьЗначениеКонстанты("КаталогПоВидам");
			Иначе 
				Родитель = crm_RetailCRMОбщий.ПолучитьЗначениеКонстанты("Каталог");
			КонецЕсли;
			Склады 	 = crm_RetailCRMОбщий.ПолучитьЗначениеКонстанты("Склады");
		КонецЕсли;
		...
		Запрос.УстановитьПараметр("ВидЦены", crm_RetailCRMОбщий.ПолучитьЗначениеКонстанты("ЗакупочнаяЦена"));
...
Процедура Мод_ОстаткиИЦеныВCRMХарактеристики() Экспорт 
	...
	Для Каждого Магазин Из СписокМагазинов Цикл 
		...
		Запрос.УстановитьПараметр("Склады",  Склады.ВыгрузитьКолонку("Склад"));
		Запрос.УстановитьПараметр("ЗакупочнаяЦена", crm_RetailCRMОбщий.ПолучитьЗначениеКонстанты("ЗакупочнаяЦена"));
		Запрос.УстановитьПараметр("ТекДата", ТекущаяДата());
...
Функция УТ11_ЗапросДляКаталога(фильтр)
	...
	Запрос.УстановитьПараметр("ЦенаЗакупа", crm_RetailCRMОбщий.ПолучитьЗначениеКонстанты("ЗакупочнаяЦена"));
	Запрос.УстановитьПараметр("ЦенаПродажи", crm_RetailCRMОбщий.ПолучитьЗначениеКонстанты("ЦенаПродажи"));
...
Процедура Мод_ВыгрузкаТиповЦенВCRM() Экспорт
	...
	ВидыЦен 			= crm_RetailCRMОбщий.ПолучитьЗначениеКонстанты("ТипыЦенCRM");
...
Процедура Мод_ВыгрузкаКаталогаВXML(табКаталога, ПутьКфайлу) Экспорт
	...
	Иначе
		...
		Иначе 
			фильтр = crm_RetailCRMОбщий.ПолучитьЗначениеКонстанты("Каталог");
		КонецЕсли;
	КонецЕсли;
	...
	Для Каждого Магазин Из СписокМагазинов Цикл 
		...
		Иначе
			Если Не ЗначениеЗаполнено(ПутьКфайлу) Тогда 
				ПутьКфайлу = crm_RetailCRMОбщий.ПолучитьЗначениеКонстанты("ПутьКФайлу");
				Если Не ЗначениеЗаполнено(ПутьКФайлу) Тогда 
...
Процедура Мод_ВыгрузкаКаталогаВXMLХарактеристики(табКаталога, ПутьКфайлу) Экспорт
	...
	ДобавлятьGUIDОрганизации = crm_RetailCRMОбщий.ПолучитьЗначениеКонстанты("GUIDОрганизации");
...
Процедура Мод_ВыгрузкаКаталогаВXMLПоВидам(табКаталога, ПутьКфайлу) Экспорт
	...
	Иначе
		...
		Если табКаталога <> Неопределено Тогда 
			...
			фильтр = ?(ЗначениеЗаполнено(фильтр), фильтр.ВыгрузитьКолонку("Номенклатура"), "");
		Иначе 
			фильтр = crm_RetailCRMОбщий.ПолучитьЗначениеКонстанты("КаталогПоВидам");
		КонецЕсли;
	КонецЕсли;
...
Процедура Мод_ВыгрузитьКлиента(Клиент) Экспорт
	...
	КодМагазина = СокрЛП(crm_RetailCRMОбщий.ПолучитьЗначениеКонстанты("КодМагазина"));
		...
		Попытка
			...
			Если Ответ1["success"] = Истина Тогда
				...
				Если СкладскаяСистемаАктивна <> Неопределено Тогда
					...
					Если crm_RetailCRMОбщий.ПолучитьЗначениеКонстанты("СкладскаяСистема") <> СкладскаяСистемаАктивна Тогда 
						Мод_СохранитьКонстанты();
						ЭтотОбъект.СкладскаяСистема = СкладскаяСистемаАктивна;
						УстановитьЗначениеКонстанты("СкладскаяСистема", СкладскаяСистемаАктивна);
					КонецЕсли;
	...
	Если СкладскаяСистемаАктивна <> Истина Тогда 
		...
		Попытка
			...
			Если Найти(ответ, """success"":true") > 0 Тогда                 
				...
				Если crm_RetailCRMОбщий.ПолучитьЗначениеКонстанты("СкладскаяСистема") <> Истина Тогда 
					Мод_СохранитьКонстанты();
					УстановитьЗначениеКонстанты("СкладскаяСистема", Истина);
				КонецЕсли;
			Иначе
				...
				Если crm_RetailCRMОбщий.ПолучитьЗначениеКонстанты("СкладскаяСистема") <> Ложь Тогда 
					Мод_СохранитьКонстанты();
					УстановитьЗначениеКонстанты("СкладскаяСистема", Ложь);
				КонецЕсли;
			КонецЕсли;
...
&НаСервере
Процедура УТ11_СоздатьВозврат(докВозврат, Реализация, Заказ, ТоварыCRM)
	...
	Исключение
		Сообщить("Не удалось заполнить: " + Реализация + " " + ОписаниеОшибки(), СтатусСообщения.Важное);
	КонецПопытки;
...
Процедура ВыгрузитьАрхивКлиентов(Партнер) Экспорт
	...
	Корпы = crm_RetailCRMОбщий.ПолучитьЗначениеКонстанты("КорпоративныеКлиенты");
	СвойствоКонтрагентов = Обработки.crm_RetailCRMОбработка.УТ11_ПолучитьСвойство("ИД", Справочники.НаборыДополнительныхРеквизитовИСведений.УдалитьСправочник_Контрагенты);
	СвойствоПартнеров = Обработки.crm_RetailCRMОбработка.УТ11_ПолучитьСвойство("ИД", Справочники.НаборыДополнительныхРеквизитовИСведений.УдалитьСправочник_Партнеры);
	СвойствоКонтЛиц = Обработки.crm_RetailCRMОбработка.УТ11_ПолучитьСвойство("ИД", Справочники.НаборыДополнительныхРеквизитовИСведений.УдалитьСправочник_КонтактныеЛицаПартнеров);
	
	СтруктураПараметров.ИмяСервера    	= СокрЛП(crm_RetailCRMОбщий.ПолучитьЗначениеКонстанты("ИмяСервера"));;
	СтруктураПараметров.КлючCRM    		= СокрЛП(crm_RetailCRMОбщий.ПолучитьЗначениеКонстанты("КлючCRM"));
	СтруктураПараметров.КодМагазина    	= СокрЛП(crm_RetailCRMОбщий.ПолучитьЗначениеКонстанты("КодМагазина"));
...
Процедура ВыгрузитьАрхивЗаказов(Организация) Экспорт
	...
	СтруктураПараметров.КлючCRM    		= СокрЛП(crm_RetailCRMОбщий.ПолучитьЗначениеКонстанты("КлючCRM"));
	СтруктураПараметров.КодМагазина    	= СокрЛП(crm_RetailCRMОбщий.ПолучитьЗначениеКонстанты("КодМагазина"));
	СтруктураПараметров.offers_id    	= crm_RetailCRMОбщий.ПолучитьЗначениеКонстанты("offers_id");
	СтруктураПараметров.ИдентифТовара	= ИдентифТовара; 
	СтруктураПараметров.ИспользованиеХарактеристик	= crm_RetailCRMОбщий.ПолучитьЗначениеКонстанты("ИспользованиеХарактеристик");
...
Процедура Мод_ЗагрузитьБрони(ИДИзменений, ТЗБрони, ИспДатыАктуальности = Ложь, ДатаИзменений) Экспорт
	...
	ИДИзменений = crm_RetailCRMОбщий.ПолучитьЗначениеКонстанты("packs_sinceId");	
	
	Если ТЗБрони <> Неопределено и ТЗБрони.Количество()>0 Тогда 
		//ЗагрузкаЗаказов(фильтрИд);
	Иначе 
...
Процедура ПроверитьНаличие (док) 
	...
	|		И НЕ Номенклатура.ЭтоГруппа
	|		И Склады.Ссылка В(&Склады)
	|		И Номенклатура.Ссылка В(&Номенклатура)) КАК ВложенныйЗапрос
	|		ЛЕВОЕ СОЕДИНЕНИЕ ТаблицаДанныхОтчета КАК Таблица
	|		ПО ВложенныйЗапрос.Номенклатура = Таблица.Номенклатура
	|			И ВложенныйЗапрос.Характеристика = Таблица.Характеристика
	|			И ВложенныйЗапрос.Склад = Таблица.Склад
...
Процедура ВыгрузитьАрхивКорпоративныхКлиентов(Партнер) Экспорт
	...
	Запрос.УстановитьПараметр("Организация", crm_RetailCRMОбщий.ПолучитьЗначениеКонстанты("Организация"));
...
Процедура ВыгрузитьАдресаКорпов(Партнер) Экспорт 
	...
	СвойствоПартнеров = Обработки.crm_RetailCRMОбработка.УТ11_ПолучитьСвойство("ИД", Справочники.НаборыДополнительныхРеквизитовИСведений.УдалитьСправочник_Партнеры);
	СвойствоАдреса = Обработки.crm_RetailCRMОбработка.УТ11_ПолучитьСвойство("АдресВыгружен", Справочники.НаборыДополнительныхРеквизитовИСведений.УдалитьСправочник_Партнеры);
