...
Функция СоздатьЗапросПоискаЗаказов() Экспорт
	...
	|ИЗ
	|	РегистрСведений.crm_ИсходныеДанныеЗаказов КАК РС
	|ГДЕ
	|	РС.id = &id
	|	и РС.site = &site";
...
Процедура УТ11_СоздатьЗаказ(ДанныеЗаказа, стр, искДок, ТЗброни = Неопределено) Экспорт
	...
	//ЗаписатьЗаказ_Вендор();	
	
	Док.ОбменДанными.Загрузка = Ложь;
	Док.Записать();
	
	
	НЗ = РегистрыСведений.crm_ИсходныеДанныеЗаказов.СоздатьНаборЗаписей();
	...
	// ЕНС. Установка свойств заказа в "Дополнительные сведения". Отключено, т.к. есть РС.crm_ИсходныеДанныеЗаказов
