...
Функция ПолучитьНомерЗаказа(номЗаказа) Экспорт
	...
	Попытка
		Префикс = ПолучитьЗначениеКонстанты("ПрефиксЗаказов");
...
Процедура ВыгрузитьЗаказВCRM(док, Отменить = Ложь) Экспорт
	...
	КодМагазина = Обработки.crm_RetailCRMОбработка.УТ11_ПолучитьЗначениеСвойстваОбъекта(Док.Ссылка, СвойствоЗаказовМагазин);
	...
	НомерЗаказа 	= Обработки.crm_RetailCRMОбработка.УТ11_ПолучитьЗначениеСвойстваОбъекта(Док.ссылка, СвойствоЗаказов);
	...
	Для каждого стр из Док["Товары"] Цикл
		...
		Если ПолучитьЗначениеКонстанты("ИспользованиеХарактеристик") = Истина Тогда
			...
			Если  ПолучитьЗначениеКонстанты("GUIDОрганизации") = Истина Тогда
	...
	Если Эталонная Тогда
		СостояниеЗаказа = Обработки.crm_RetailCRMОбработка.УТ11_ПолучитьСостояниеЗаказа(док);
		...
		ТабСостояний = ПолучитьЗначениеКонстанты("ТабСостояний");
...
Процедура ВыгрузитьОплатуВCRM(докОплаты, Отменить = Ложь, ИДОплаты,ТипДокументаОплаты, КодОплаты) Экспорт  	
	...
	Если Не ЗначениеЗаполнено(КодОплаты) Тогда 
		ТипыОплат = ПолучитьЗначениеКонстанты("СоответствиеТиповОплатИз1С");
	...
	Иначе 
		Для каждого стр Из докОплаты.РасшифровкаПлатежа Цикл 
			...
			Если ЗначениеЗаполнено(докЗаказа) и ТипЗнч(докЗаказа) = Тип("ДокументСсылка.ЗаказКлиента") Тогда //и Лев(докЗаказа.Комментарий,1) = "#" Тогда
				...
				СвойствоЗаказовМагазин = Обработки.crm_RetailCRMОбработка.УТ11_ПолучитьСвойство("site", Справочники.НаборыДополнительныхРеквизитовИСведений.УдалитьДокумент_ЗаказКлиента);
				КодМагазина = Обработки.crm_RetailCRMОбработка.УТ11_ПолучитьЗначениеСвойстваОбъекта(докЗаказа.Ссылка, СвойствоЗаказовМагазин);
				
				СвойствоЗаказов = Обработки.crm_RetailCRMОбработка.УТ11_ПолучитьСвойство("ИД", Справочники.НаборыДополнительныхРеквизитовИСведений.УдалитьДокумент_ЗаказКлиента);
				НомерЗаказа 	= Обработки.crm_RetailCRMОбработка.УТ11_ПолучитьЗначениеСвойстваОбъекта(докЗаказа.ссылка, СвойствоЗаказов);
				НомерЗаказа 	= СокрЛП(Формат(НомерЗаказа, "ЧГ=0"));
				...
				Если не ЗначениеЗаполнено(ИДОплаты) или ИДОплаты = "" Тогда 
					...
					Если Найти(Ответ, "success"":true") > 0 Тогда
						...
						Если ТипЗнч(докОплаты) = Тип("ДокументСсылка.ПриходныйКассовыйОрдер") Тогда 
							...
							СвойствоТип = Обработки.crm_RetailCRMОбработка.УТ11_ПолучитьСвойство("ТипОплаты", Справочники.НаборыДополнительныхРеквизитовИСведений.УдалитьДокумент_ПриходныйКассовыйОрдер);
						...
						КонецЕсли;
						Обработки.crm_RetailCRMОбработка.УТ11_УстановитьСвойствоУОбъекта(докОплаты.Ссылка, СвойствоОплаты, ИДОплаты);
						Обработки.crm_RetailCRMОбработка.УТ11_УстановитьСвойствоУОбъекта(докОплаты.Ссылка, СвойствоТип, КодОплаты);
					Иначе
		...
		Для каждого стр Из докВозврОплаты.РасшифровкаПлатежа Цикл 
			...
			Если ЗначениеЗаполнено(докЗаказа) Тогда //теперь получаем все оплаты
				...
				Если СписокОплат <> Неопределено и СписокОплат.Количество() = 1 Тогда 
					...
					Если ТипДокументаОплаты = "Эквайринговая операция" Тогда 
						СвойствоОплатыВозврат = Обработки.crm_RetailCRMОбработка.УТ11_ПолучитьСвойство("ИДоплатыВозврат", Справочники.НаборыДополнительныхРеквизитовИСведений.УдалитьДокумент_ОперацияПоПлатежнойКарте);
						ИДОплатыВозврат = Обработки.crm_RetailCRMОбработка.УТ11_ПолучитьЗначениеСвойстваОбъекта(докВозврОплаты,СвойствоОплатыВозврат);
					ИначеЕсли ТипДокументаОплаты = "Расходный кассовый ордер" Тогда
						СвойствоОплатыВозврат = Обработки.crm_RetailCRMОбработка.УТ11_ПолучитьСвойство("ИДоплатыВозврат", Справочники.НаборыДополнительныхРеквизитовИСведений.УдалитьДокумент_РасходныйКассовыйОрдер);
						ИДОплатыВозврат = Обработки.crm_RetailCRMОбработка.УТ11_ПолучитьЗначениеСвойстваОбъекта(докВозврОплаты,СвойствоОплатыВозврат);
					ИначеЕсли ТипДокументаОплаты = "Списание безналичных ДС" Тогда
						СвойствоОплатыВозврат = Обработки.crm_RetailCRMОбработка.УТ11_ПолучитьСвойство("ИДоплатыВозврат", Справочники.НаборыДополнительныхРеквизитовИСведений.УдалитьДокумент_СписаниеБезналичныхДенежныхСредств);
						ИДОплатыВозврат = Обработки.crm_RetailCRMОбработка.УТ11_ПолучитьЗначениеСвойстваОбъекта(докВозврОплаты,СвойствоОплатыВозврат);
					КонецЕсли;
					...
					Если Найти(Ответ2, "success"":true") > 0 Тогда
						...
						Если ТипЗнч(докВозврОплаты) = Тип("ДокументСсылка.РасходныйКассовыйОрдер") Тогда 
							СвойствоОплатыВозврат = Обработки.crm_RetailCRMОбработка.УТ11_ПолучитьСвойство("ИДИзмененнойОплаты", Справочники.НаборыДополнительныхРеквизитовИСведений.УдалитьДокумент_РасходныйКассовыйОрдер);
						...
						ИначеЕсли ТипЗнч(докВозврОплаты) = Тип("ДокументСсылка.СписаниеБезналичныхДенежныхСредств") Тогда 
							СвойствоОплатыВозврат = Обработки.crm_RetailCRMОбработка.УТ11_ПолучитьСвойство("ИДИзмененнойОплаты", Справочники.НаборыДополнительныхРеквизитовИСведений.УдалитьДокумент_СписаниеБезналичныхДенежныхСредств);
							СвойствоТип = Обработки.crm_RetailCRMОбработка.УТ11_ПолучитьСвойство("ТипОплаты", Справочники.НаборыДополнительныхРеквизитовИСведений.УдалитьДокумент_СписаниеБезналичныхДенежныхСредств);
						ИначеЕсли ТипЗнч(докВозврОплаты) = Тип("ДокументСсылка.ОперацияПоПлатежнойКарте") Тогда 
							СвойствоОплатыВозврат = Обработки.crm_RetailCRMОбработка.УТ11_ПолучитьСвойство("ИДИзмененнойОплаты", Справочники.НаборыДополнительныхРеквизитовИСведений.УдалитьДокумент_ОперацияПоПлатежнойКарте);
							СвойствоТип = Обработки.crm_RetailCRMОбработка.УТ11_ПолучитьСвойство("ТипОплаты", Справочники.НаборыДополнительныхРеквизитовИСведений.УдалитьДокумент_ОперацияПоПлатежнойКарте);	
						КонецЕсли;
						
						Обработки.crm_RetailCRMОбработка.УТ11_УстановитьСвойствоУОбъекта(докВозврОплаты.Ссылка, СвойствоОплатыВозврат, ИДОплаты);
						Обработки.crm_RetailCRMОбработка.УТ11_УстановитьСвойствоУОбъекта(докВозврОплаты.Ссылка, СвойствоТип, КодОплаты);
						
						Если Отменить = Истина Тогда
                        	Обработки.crm_RetailCRMОбработка.УТ11_УстановитьСвойствоУОбъекта(докВозврОплаты.Ссылка, СвойствоОплатыВозврат, "");
							Обработки.crm_RetailCRMОбработка.УТ11_УстановитьСвойствоУОбъекта(докВозврОплаты.Ссылка, СвойствоТип, "");
						КонецЕсли;
						...
						Если НЕ ЗначениеЗаполнено(ИДОплатыВозврат) Тогда
							...
							Если Найти(Ответ, "success"":true") > 0 Тогда
								...
								ИначеЕсли ТипЗнч(докВозврОплаты) = Тип("ДокументСсылка.ОперацияПоПлатежнойКарте") Тогда 
									...
									СвойствоТип = Обработки.crm_RetailCRMОбработка.УТ11_ПолучитьСвойство("ТипОплаты", Справочники.НаборыДополнительныхРеквизитовИСведений.УдалитьДокумент_ОперацияПоПлатежнойКарте);	
								КонецЕсли;
								Обработки.crm_RetailCRMОбработка.УТ11_УстановитьСвойствоУОбъекта(докВозврОплаты.Ссылка, СвойствоОплатыВозврат, ИДОплатыВозврат);
								Обработки.crm_RetailCRMОбработка.УТ11_УстановитьСвойствоУОбъекта(докВозврОплаты.Ссылка, СвойствоТип, КодОплаты);
							Иначе
...
Процедура ВыгрузитьАрхивныйЗаказВCRM(ДокЗаказ, ТЗРеализации, ТЗВозвратов,ТЗПКО,ТЗБезнал,ТЗЭквайринг, СтруктураПараметров, Отказ = Ложь, СчетчикАрхива, МассивПакета, КолЗаказов) Экспорт
	...
	//получаем id клиента
	ИДКлиента = Обработки.crm_RetailCRMОбработка.УТ11_ПолучитьЗначениеСвойстваОбъекта(ДокЗаказ.Контрагент, СтруктураПараметров.СвойствоКонтрагентов);
		...
		Если КИ.Тип = Перечисления.ТипыКонтактнойИнформации.Адрес Тогда 
			...
			Если Найти(ответ, """success"":true") > 0 Тогда
				...
				Попытка
					...
					Обработки.crm_RetailCRMОбработка.УТ11_УстановитьСвойствоУОбъекта(Данные.Ссылка, СвойствоАдреса, "Истина");
				Исключение
...
Процедура УстановитьИДПартнеров(КлиентCRM, СвойствоПартнеров) Экспорт 
	...
	Если ИДКлиента <> 0 Тогда
		...
		Если ЗначениеЗаполнено(искПартнер) и ИДКлиента <> 0 Тогда
			Попытка
			Обработки.crm_RetailCRMОбработка.УТ11_УстановитьСвойствоУОбъекта(искПартнер, СвойствоПартнеров, Формат(ИДКлиента, "ЧГ=0"));
...
Процедура УстановитьИДКонтрагентов(КлиентCRM, СвойствоКонтрагентов) Экспорт 
	...
	Если ИДКлиента <> 0 Тогда
		...
		Если ЗначениеЗаполнено(искКонтр) и ИДКлиента <> 0 Тогда
			Попытка
			Обработки.crm_RetailCRMОбработка.УТ11_УстановитьСвойствоУОбъекта(искКонтр, СвойствоКонтрагентов, Формат(ИДКлиента, "ЧГ=0"));
...
Процедура УстановитьИДКонтЛиц(КлиентCRM, СвойствоКонтЛиц) Экспорт 
	...
	Если ИДКлиента <> 0 Тогда
		...
		Если ЗначениеЗаполнено(искКонт) и ИДКлиента <> 0 Тогда
			Попытка
			Обработки.crm_RetailCRMОбработка.УТ11_УстановитьСвойствоУОбъекта(искКонт, СвойствоКонтЛиц, Формат(ИДКлиента, "ЧГ=0"));
...
Процедура УстановитьИДЗаказов(заказCRM, Структура) Экспорт 
	...
	Если ИДЗаказа <> 0 Тогда
		...
		Если ЗначениеЗаполнено(искЗаказ) и ИДЗаказа <> 0 Тогда
			Обработки.crm_RetailCRMОбработка.УТ11_УстановитьСвойствоУОбъекта(искЗаказ, Структура.СвойствоЗаказов, Формат(ИДЗаказа, "ЧГ=0"));
			Обработки.crm_RetailCRMОбработка.УТ11_УстановитьСвойствоУОбъекта(искЗаказ, Структура.СвойствоЗаказовМагазин, Структура.КодМагазина);
		КонецЕсли;
	КонецЕсли;
	...
	Если ИДКлиента <> 0 Тогда
		...
		Если ЗначениеЗаполнено(искКонтр) Тогда
			...
			Если ЗначениеЗаполнено(искПарт) Тогда
				Попытка
					Обработки.crm_RetailCRMОбработка.УТ11_УстановитьСвойствоУОбъекта(искПарт, СвойствоПартнеров, Формат(ИДКлиента, "ЧГ=0"));
...
Процедура ВыгрузитьРеализациюВCRM(Реализация, Отменить=Ложь) Экспорт
	...
	ДокВозврат = Обработки.crm_RetailCRMОбработка.УТ11_ВернутьВозврат(Реализация.Ссылка);
	...
	crm_СтатусЗаказа = Обработки.crm_RetailCRMОбработка.УТ11_ПолучитьЗначениеСвойстваОбъекта(Реализация.Ссылка, СвойствоРеализаций);  
	...
	Если Найти(ответ, """success"":true") > 0 Тогда
		...
		Если СтатусЗаказа = "partially-completed" Тогда 
			СвойствоРеализаций = Обработки.crm_RetailCRMОбработка.УТ11_ПолучитьСвойство("crm_СтатусЗаказа", Справочники.НаборыДополнительныхРеквизитовИСведений.УдалитьДокумент_РеализацияТоваровУслуг, "Сведение");
			Обработки.crm_RetailCRMОбработка.УТ11_УстановитьСвойствоУОбъекта(Реализация.Ссылка, СвойствоРеализаций, СтатусЗаказа);  
		КонецЕсли;
	Иначе
...
Процедура ВыгрузитьВозвратВCRM(докВозврат, Отменить=Ложь) Экспорт
	...
	//поиск заказов по ИД (доп. сведение)
	СвойствоЗаказовМагазин = Обработки.crm_RetailCRMОбработка.УТ11_ПолучитьСвойство("site", Справочники.НаборыДополнительныхРеквизитовИСведений.УдалитьДокумент_ЗаказКлиента);
	КодМагазина = Обработки.crm_RetailCRMОбработка.УТ11_ПолучитьЗначениеСвойстваОбъекта(ЗаказКлиента.Ссылка, СвойствоЗаказовМагазин);
	
	переменные.Вставить("site", КодМагазина);
	
	СвойствоЗаказов = Обработки.crm_RetailCRMОбработка.УТ11_ПолучитьСвойство("ИД", Справочники.НаборыДополнительныхРеквизитовИСведений.УдалитьДокумент_ЗаказКлиента);
	НомерЗаказа 	= Обработки.crm_RetailCRMОбработка.УТ11_ПолучитьЗначениеСвойстваОбъекта(ЗаказКлиента.ссылка, СвойствоЗаказов);
	НомерЗаказа 	= СокрЛП(Формат(НомерЗаказа, "ЧГ=0"));
...
Процедура СоздатьКлиентаИзФормыКонтрагента(Партнер, Успешно, Создать, Обновить) Экспорт 
	...
	ИДПартнера 	= Обработки.crm_RetailCRMОбработка.УТ11_ПолучитьЗначениеСвойстваОбъекта(Партнер, СвойствоПартнеров);
	ИДКонтрагента = Обработки.crm_RetailCRMОбработка.УТ11_ПолучитьЗначениеСвойстваОбъекта(Партнер, СвойствоПартнеров);
...
Функция АктивироватьКлиентаВПЛ (Партнер, Успешно) Экспорт 
	...
	СвойствоПартнеров = Обработки.crm_RetailCRMОбработка.УТ11_ПолучитьСвойство("ИДУчастияВПЛ", Справочники.НаборыДополнительныхРеквизитовИСведений.УдалитьСправочник_Партнеры);
	ИДУчастия 	= Обработки.crm_RetailCRMОбработка.УТ11_ПолучитьЗначениеСвойстваОбъекта(Партнер, СвойствоПартнеров);
	...
	Если ответ["success"] = Истина Тогда
		Успешно = Истина;
		СвойствоПартнеров = Обработки.crm_RetailCRMОбработка.УТ11_ПолучитьСвойство("АктивенВПЛ", Справочники.НаборыДополнительныхРеквизитовИСведений.УдалитьСправочник_Партнеры);
		Обработки.crm_RetailCRMОбработка.УТ11_УстановитьСвойствоУОбъекта(Партнер, СвойствоПартнеров, ответ["loyaltyAccount"]["active"]);
		Сообщить("Клиент активирован в программе лояльности");
	Иначе 
		Если ответ["errorMsg"] = "LoyaltyAccount has been already activated" Тогда 
			СвойствоПартнеров = Обработки.crm_RetailCRMОбработка.УТ11_ПолучитьСвойство("АктивенВПЛ", Справочники.НаборыДополнительныхРеквизитовИСведений.УдалитьСправочник_Партнеры);
			Обработки.crm_RetailCRMОбработка.УТ11_УстановитьСвойствоУОбъекта(Партнер, СвойствоПартнеров, Истина);
			Успешно = Истина;
...
Процедура УстановитьСвойстваПЛ(Партнер, idУчастия, Активность) Экспорт
	...
	СвойствоПартнеров = Обработки.crm_RetailCRMОбработка.УТ11_ПолучитьСвойство("ИДУчастияВПЛ", Справочники.НаборыДополнительныхРеквизитовИСведений.УдалитьСправочник_Партнеры);
	Обработки.crm_RetailCRMОбработка.УТ11_УстановитьСвойствоУОбъекта(Партнер, СвойствоПартнеров, idУчастия);
	
	СвойствоПартнеров = Обработки.crm_RetailCRMОбработка.УТ11_ПолучитьСвойство("АктивенВПЛ", Справочники.НаборыДополнительныхРеквизитовИСведений.УдалитьСправочник_Партнеры);
	Обработки.crm_RetailCRMОбработка.УТ11_УстановитьСвойствоУОбъекта(Партнер, СвойствоПартнеров, Активность);
...
Процедура ПолучитьДанныеБонусногоСчета (Партнер, Структура) Экспорт
	...
	Если Ответ["success"] = Истина Тогда
		...
		Если Строка(Ответ["loyaltyAccount"]["loyalty"]["id"]) = ПолучитьЗначениеКонстанты("idПЛ") Тогда 
			Статус = "";
...
Процедура ОбработатьОтветЗагрузкиКлиента(ответ,СтруктураПараметров, Партнер, Контрагент, Успешно);
	...
	Если ответ["errorMsg"] = "Customer already exists" Тогда  
		...
		Если Ответ["success"] = истина Тогда 
			...
			Обработки.crm_RetailCRMОбработка.УТ11_УстановитьСвойствоУОбъекта(Партнер,  СтруктураПараметров.СвойствоПартнеров, Формат(Ответ["customer"]["id"], "ЧГ=0"));
			Обработки.crm_RetailCRMОбработка.УТ11_УстановитьСвойствоУОбъекта(Контрагент,  СтруктураПараметров.СвойствоКонтрагентов, Формат(Ответ["customer"]["id"], "ЧГ=0"));
			Успешно = Истина;
...
Процедура СоздатьКлиентаВCRM (Выборка, Успешно, id, Создать, Обновить, КодМагазина, Ответ) Экспорт 
	...
	Если ответ["success"] = Истина Тогда
		...
		Если Создать Тогда
			Обработки.crm_RetailCRMОбработка.УТ11_УстановитьСвойствоУОбъекта(Выборка.Партнер, СвойствоПартнеров, id); 
			Обработки.crm_RetailCRMОбработка.УТ11_УстановитьСвойствоУОбъекта(Выборка.Контрагент, СвойствоКонтрагентов, id);
			...
			Если ответ["errors"]["loyalty"] = "The customer is in this loyalty program already" Тогда 
				...
				Если Ответ1["success"] = Истина Тогда 
					...
					Для Каждого Участие из Ответ1["loyaltyAccounts"] Цикл 
						Если Строка(Участие["loyalty"]["id"]) = ПолучитьЗначениеКонстанты("idПЛ") Тогда 
...
Процедура ВыгрузитьБрониВCRM(док);
	...
	НоменклатураДоставки = ПолучитьЗначениеКонстанты("Доставка");
	Склады = ПолучитьЗначениеКонстанты("Склады");
...
Функция СоздатьКлиента1С (Структура) Экспорт 
	...
	КонецЕсли;
	Партнер.ГруппаДоступа = crm_RetailCRMОбщий.ПолучитьЗначениеКонстанты("ГруппаДоступаПартнеров");
	...
	Если ЗначениеЗаполнено(Структура.IDКлиента) Тогда 
		СвойствоПартнеров = Обработки.crm_RetailCRMОбработка.УТ11_ПолучитьСвойство("ИД", Справочники.НаборыДополнительныхРеквизитовИСведений.УдалитьСправочник_Партнеры);
		Обработки.crm_RetailCRMОбработка.УТ11_УстановитьСвойствоУОбъекта(Партнер.Ссылка, СвойствоПартнеров, Структура.IDКлиента);  
		// 

		//запись id контрагента
		СвойствоКонтрагентов = Обработки.crm_RetailCRMОбработка.УТ11_ПолучитьСвойство("ИД", Справочники.НаборыДополнительныхРеквизитовИСведений.УдалитьСправочник_Контрагенты);
		Обработки.crm_RetailCRMОбработка.УТ11_УстановитьСвойствоУОбъекта(Контрагент.Ссылка, СвойствоКонтрагентов, Структура.IDКлиента);  
	КонецЕсли;
	//
...
Функция РассчитатьСкидкуПоПЛ(ДанныеПЛ, Магазин, Товары, БонусовКСписанию) Экспорт 
	...
	Для Каждого стр Из Товары Цикл 
		...
		Если ЗначениеЗаполнено(КодТипаЦены1) и ПолучитьЗначениеКонстанты("ИспользованиеТиповЦен") Тогда 
			ТипЦены = Новый Соответствие();
...
Процедура ВыгрузитьЧекВCRM(док, Отменить = Ложь, ДанныеПЛ = Неопределено, Действие, ДанныеЧека, ИДЗаказа, ФИО) Экспорт
	...
	Если crm_RetailCRMОбщий.ПолучитьЗначениеКонстанты("ВыгружатьЧек") = Ложь Тогда 
		Возврат;
	КонецЕсли;
	...
	Если Действие <> "ВыгрузитьНовыйЧек" Тогда
		...
		Если док.Партнер = Справочники.Партнеры.РозничныйПокупатель или док.Партнер.Наименование = "Розничный покупатель" Тогда 
			Если crm_RetailCRMОбщий.ПолучитьЗначениеКонстанты("ВыгружатьЧекРП") = Ложь Тогда 
				Возврат;
			КонецЕсли;
		Иначе 
			//получим id партнера
			СвойствоПартнеров = Обработки.crm_RetailCRMОбработка.УТ11_ПолучитьСвойство("ИД", Справочники.НаборыДополнительныхРеквизитовИСведений.УдалитьСправочник_Партнеры);
			ИДПартнера 	= Обработки.crm_RetailCRMОбработка.УТ11_ПолучитьЗначениеСвойстваОбъекта(док.Партнер, СвойствоПартнеров);
		КонецЕсли;
	...
	Если Действие = "ВыгрузитьЧекБезПЛ" Тогда
		...
		СписокФормОплаты	= ПолучитьЗначениеКонстанты("СоответствиеФормОплаты");
		Оплаты = Новый Массив();
