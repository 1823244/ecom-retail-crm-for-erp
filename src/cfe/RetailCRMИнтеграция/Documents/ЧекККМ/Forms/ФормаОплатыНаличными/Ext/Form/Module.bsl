...
Процедура crm_ПередЗакрытиемПосле(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	//
	//Если crm_RetailCRMОбщий.ПолучитьЗначениеКонстанты("ВыгружатьЧек") = Истина и crm_RetailCRMОбщий.ПолучитьЗначениеКонстанты("ИспользованиеПЛ") = Ложь Тогда
	...
	//Если crm_RetailCRMОбщий.ПолучитьЗначениеКонстанты("ВыгружатьЧеки") = Истина Тогда
	//	Если ЭтотОбъект.СуммаДоплаты = 0 и ЭтотОбъект.РезультатОплаты <> "Отмена" Тогда 
