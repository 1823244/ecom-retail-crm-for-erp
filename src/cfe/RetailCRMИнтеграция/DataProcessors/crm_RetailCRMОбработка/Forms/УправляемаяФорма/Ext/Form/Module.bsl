...
Процедура СохранитьКонстанты(Команда)
	...
	Иначе 
		Если crm_RetailCRMОбщий.ПолучитьЗначениеКонстанты("ИерархияНоменклатуры") = "Виды номенклатуры" Тогда
			ЭтаФорма.Элементы.ТабКаталог.Видимость = Ложь;
	...
	Если crm_RetailCRMОбщий.ПолучитьЗначениеКонстанты("ВыгрузкаОплатВCRM") <> Истина и
		crm_RetailCRMОбщий.ПолучитьЗначениеКонстанты("СозданиеОплат") <> Истина Тогда
		ЭтаФорма.Элементы.Оплаты.Видимость = Ложь;
	...
	//++ Эталонка
	Если crm_RetailCRMОбщий.ПолучитьЗначениеКонстанты("Эталонная") = Истина Тогда 
		...
		Если crm_RetailCRMОбщий.ПолучитьЗначениеКонстанты("СозданиеВозвратов") = Истина Тогда
	...
	Если crm_RetailCRMОбщий.ПолучитьЗначениеКонстанты("РаботаСРозницей") = Истина Тогда
		ЭтаФорма.Элементы.Розница.Видимость = Истина;
...
Процедура ПриОткрытии(Отказ)
	...
	Если Не ЗначениеЗаполнено(Объект.НазваниеПЛ) Тогда 
		Объект.НазваниеПЛ = crm_RetailCRMОбщий.ПолучитьЗначениеКонстанты("НазваниеПЛ");
	КонецЕсли;
	...
	Если crm_RetailCRMОбщий.ПолучитьЗначениеКонстанты("Эталонная") = Истина Тогда 
		...
		Если crm_RetailCRMОбщий.ПолучитьЗначениеКонстанты("СозданиеРеализаций") Тогда
			ЭтаФорма.Элементы.ТабСтатусыРеализация.Видимость = Истина;
		Иначе 
			ЭтаФорма.Элементы.ТабСтатусыРеализация.Видимость = Ложь;
		КонецЕсли; 
	...
	Если crm_RetailCRMОбщий.ПолучитьЗначениеКонстанты("ИспользованиеПЛ") = Истина Тогда 
		ЭтаФорма.Элементы.ПрограммаЛояльности.Видимость = Истина;
	Иначе 
		ЭтаФорма.Элементы.ПрограммаЛояльности.Видимость = Ложь;
	КонецЕсли;
	...
	Если crm_RetailCRMОбщий.ПолучитьЗначениеКонстанты("ВыгружатьЧек") = Истина Тогда 
		ЭтаФорма.Элементы.ВыгружатьЧекРП.Доступность = Истина;
		ЭтаФорма.Элементы.ВыгружатьЧекВозврат.Доступность = Истина;
	...
	СвязьМагазина = crm_RetailCRMОбщий.ПолучитьЗначениеКонстанты("СвязьМагазина"); 
	...
	СкладскаяСистемаАктивна = crm_RetailCRMОбщий.ПолучитьЗначениеКонстанты("СкладскаяСистема");
...
Процедура ЗагрузитьЗаказыНаСервере()
	...
	Обработка = РеквизитФормыВЗначение("Объект");
	Обработка.Мод_ЗагрузитьЗаказы(ЭтаФорма.ИспользоватьДатуАктуальности, Объект.ДатаАктуальности, Объект.ИДПоследнихИзменений);
...
Процедура ВыгрузитьКлиентовНаСервере()
	...
	Если crm_RetailCRMОбщий.ПолучитьЗначениеКонстанты("КорпоративныеКлиенты") Тогда  
...
Процедура ВкладкиПриСменеСтраницыНаСервере(СписокПЛ) 
	...
	СтруктураОтбора = Новый Структура("idПрограммы", crm_RetailCRMОбщий.ПолучитьЗначениеКонстанты("idПЛ"));// задаем условия
...
Процедура ЗаполнитьСостояниеЗаказа() 
	...
	ИначеЕсли  ТекСтрока.Статус1С = Перечисления.СтатусыЗаказовКлиентов.КОбеспечению Тогда
		...
		Если ТекСтрока.ВариантОбеспечения = Перечисления.ВариантыОбеспечения.НеТребуется или 
			ТекСтрока.ВариантОбеспечения = Перечисления.ВариантыОбеспечения.ГотовКОбеспечению Тогда 
...
Процедура ПолучитьТипыОплат(ТипыОплат) 
	...
	КодМагазина   = crm_RetailCRMОбщий.ПолучитьЗначениеКонстанты("КодМагазина");
