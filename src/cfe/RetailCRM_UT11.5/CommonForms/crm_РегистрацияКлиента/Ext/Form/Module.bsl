...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(IDКлиента) Тогда 
		Контрагент = Обработки.crm_RetailCRMОбработка.УТ11_ВернутьКонтрагентаПоID(Формат(Число(IDКлиента),"ЧГ="), "customer");
	КонецЕсли;
	
	Если Не ЗначениеЗаполнено(Контрагент) Тогда 
		Контрагент = Обработки.crm_RetailCRMОбработка.УТ11_ВернутьКонтрагентаПоТелефону_Почте("", Телефон);
