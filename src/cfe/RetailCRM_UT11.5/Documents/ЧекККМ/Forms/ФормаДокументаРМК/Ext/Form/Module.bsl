...
Процедура crm_ПриОткрытииПосле(Отказ)
	...
	ПЛ = crm_RetailCRMОбщий.ПолучитьЗначениеКонстанты("ИспользованиеПЛ");
	Если ПЛ = Истина и crm_RetailCRMОбщий.ПолучитьЗначениеКонстанты("ВыгружатьЧек") = Истина Тогда
...
Процедура ЗаменитьПартнера(ДанныеПЛ)Экспорт 
	...
	Если Объект.Партнер = Справочники.Партнеры.РозничныйПокупатель Тогда 
		...
		Партнер = Обработки.crm_RetailCRMОбработка.УТ11_ВернутьПартнераПоID(Формат(Число(ДанныеПЛ.IDКлиента),"ЧГ=0"), "customer");
		...
		Иначе 
			...
			Партнер = Обработки.crm_RetailCRMОбработка.УТ11_ВернутьКонтрагентаПоТелефону_Почте("", ДанныеПЛ.НомерТелефона);
			Если ЗначениеЗаполнено(Партнер) Тогда
				Объект.Партнер = Партнер;
			КонецЕсли;
		КонецЕсли;	
